...
\noindent\textbf{Summary.} We compare an RGB baseline against several
multispectral approaches for land-use classification on Sentinel-2 imagery.
Rather than optimise a single number on a saturated benchmark, we treat the
RGB-versus-multispectral question as a controlled experiment. We find that
spectral information helps \emph{conditionally}: strongly for classical features
($+12$~pp) and for in-domain-pretrained or from-scratch networks, but
negligibly---or harmfully---for ImageNet-pretrained networks, especially with
few labels. The value of extra bands depends on the representation that consumes
them.
...
infrared (SWIR) channels that encode surface properties invisible to the eye. We
...
budget. We therefore build an RGB baseline and several multispectral approaches,
...
reflectance scaled by $10^4$~\cite{helber2019,helber2018}. We use the all-band
...
the \emph{last} channel rather than in canonical order---a silent index footgun,
guarded here by a single named band map with unit tests. Second, band B10
(cirrus) is near-empty over these cloud-screened patches (train mean
$12.1$~DN versus $946$--$2299$ for surface bands), so we exclude it and label the
multispectral arm ``12-band''.

We draw a stratified 80/20 train/test split with a committed manifest and hold
...
significance test valid. Reflectance channels are $z$-scored using
train-split statistics; the in-domain-pretrained arms instead divide by $10^4$ to
match their pretraining distribution, so normalisation is a per-arm property. We
derive four spectral indices---NDVI, NDWI~\cite{mcfeeters1996}, NDBI and the
red-edge NDRE---from raw DN (normalised differences are scale-invariant),
$\varepsilon$-guarded and clipped to $[-1,1]$. Augmentation is restricted to
flips and $90^\circ$ rotations: photometric jitter would corrupt the physical
meaning of reflectance and the indices derived from it.

\begin{figure}[t]
\centering
\includegraphics[width=0.86\textwidth]{band_signatures.png}
...
\textbf{Tier~2 (deep)} fine-tunes a ResNet-18~\cite{he2016} under one frozen
recipe shared across all arms (AdamW, cosine schedule, $\le 30$ epochs, early
stopping on validation macro-F1), so arms differ only in inputs and pretraining,
never in tuning. Arm \textbf{E1} is the ImageNet-pretrained RGB baseline; \textbf{E2}
ingests the 12 bands by inflating the first convolution---timm replicates the
pretrained RGB filters across channels and rescales by $3/N$ to preserve
activation magnitude~\cite{carreira2017,wightman2019}; \textbf{E3} appends the four
indices to RGB. To separate \emph{pretraining domain} from \emph{spectral
information}, \textbf{E5a/E5b} repeat RGB and 12-band from scratch (no
pretraining), and \textbf{E7/E7b} use SSL4EO-S12 weights---a ResNet-18
self-supervised on Sentinel-2~\cite{wang2023,stewart2022}---for the 13-band and
RGB inputs. E1/E2 and E7b/E7 together form a $\{$ImageNet, SSL4EO$\}\times\{$RGB,
all-band$\}$ design (Fig.~\ref{fig:design}). As an optional extension we segment
water by thresholding NDWI/MNDWI~\cite{xu2006} with a fixed cut and with
Otsu's method~\cite{otsu1979}.
...
\begin{tikzpicture}[font=\footnotesize, >=Stealth,
  ...
  an/.style={box, fill=violet!10}]
\node[data] (d) {EuroSAT-MS\\27{,}000$\times$13-band\\GeoTIFF (L1C TOA)};
\node[proc, right=8mm of d] (p) {Preprocess\\stratified 80/20\\$z$-score $\mid$ indices\\(committed manifest)};
\node[tier, above right=3mm and 9mm of p] (t1) {Tier 1: Random Forest\\C1 RGB $\cdot$ C2 12-band+indices};
\node[tier, below right=3mm and 9mm of p] (t2) {Tier 2: ResNet-18\\E1\,E2\,E3 $\cdot$ E5 scratch $\cdot$ E7 SSL4EO};
\node[ev, right=30mm of p] (e) {Shared eval\\acc, macro-F1\\McNemar (Holm)};
\node[an, right=8mm of e] (a) {Analysis\\per-class $\cdot$ data-efficiency\\Grad-CAM $\cdot$ gallery};
\node[box, fill=cyan!10, below=10mm of e] (seg) {Extension:\\NDWI/MNDWI + Otsu\\water segmentation};
\draw[->] (d)--(p);
\draw[->] (p.east)-- (t1.west);
\draw[->] (p.east)-- (t2.west);
...
We follow the mandated 80/20 protocol; the validation slice is used for model
...
(point estimates) under a fixed compute budget. We report accuracy (mandated)
...
0.4~pp at 98.3--98.7\%; E3 (RGB+indices) is nominally best at
$98.69\pm0.07$\%. McNemar for E1 versus E2 yields 109--151 discordant test
patches and reaches significance in only one of three seeds after correction:
at full data RGB and the 12-band model are statistically indistinguishable---the
...
\textbf{The 2$\times$2} (Table~\ref{tab:full}, lower block) is decisive: under
SSL4EO Sentinel-2 pretraining the all-band model beats RGB ($97.2$ vs
$94.4$\%, $+2.8$~pp)---the reverse of the ImageNet rows where the two tie.
Training from scratch agrees: 12-band (E5b, $98.2$\%) beats RGB (E5a,
$96.5$\%). The subjective gallery (Fig.~\ref{fig:gallery}) shows the same classes
the confusion analysis flags---water and crop types---separating cleanly in
false-colour and NDWI views that RGB lacks.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.66\textwidth]{data_efficiency.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\textwidth]{report_gallery.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.62\textwidth]{per_class_delta__C2_multispectral.png}
...
from RGB-shaped filters with too few examples. This reconciles our study with
...
crop types through the red edge---which is exactly the information an RGB-only
...
reported standard deviations are indicative rather than inferential---our
significance claims rest on McNemar, not on overlapping error bars---and the
supporting arms are single-seed point estimates. The segmentation evaluation uses
only 11 patches with thin rivers, so its variance is high and it should be read
as illustrative. Natural extensions are a spatial-holdout evaluation, larger or
transformer backbones, L2A surface-reflectance inputs, more seeds, and
semi-supervised methods for the low-label regime~\cite{gomez2021}.
...
\texttt{scripts/download\_data.py} fetches the checksum-verified dataset and
\texttt{scripts/run\_all.sh} reproduces every result; test suites guard band
order, split integrity and label subsampling.
