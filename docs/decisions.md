...
indices, confirming the engineered indices carry real signal. This large Tier-1 gain contrasts with the small/near-zero gain expected for
...
every arm, so arms differ only in inputs and pretraining, not in tuning.
(The recipe was selected on E1's validation; an arm-specific learning-rate check on E2 is left to future work.)
...
(`tests/test_run_completeness.py`). The diagnostic signal was
